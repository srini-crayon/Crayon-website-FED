...
export function UnifiedAuthModal({
  ...
  const handleLogin = async () => {
    ...
    if (result.success) {
      // If MFA is required, don't close modal, just update UI
  ...
  const handleSignup = async () => {
    ...
    if (!validatePassword(password).isValid) {
      useAuthStore.setState({ error: "Password does not meet requirements" })
      return
    }
  ...
  const { mfaRequired } = useAuthStore()
  ...
  const isFormValid = () => {
    ...
    if (authMode === "login") {
      ...
      if (password && !validatePassword(password).isValid) return false
    ...
    <div className="space-y-2">
      ...
      {authMode === "signup" && password && (
        <PasswordStrengthIndicator
          ...
          requirements={PASSWORD_REQUIREMENTS.map(req => ({ ...req, met: req.test(password) }))}
        />
      )}
    </div>
      ...
      >
        <div className="flex flex-col items-center justify-start w-full min-h-full py-8">
          <ModalWrapper
            isOpen={isOpen}
            onClose={onClose}
            showOverlay={authMode === "signup" && !mfaRequired}
            onOverlayAction={() => {
              setAuthMode("login")
              clearError()
            }}
            showLoginOverlay={authMode === "login" && !mfaRequired}
          ...
          >
            <div className="space-y-4">
              ...
              <div className="space-y-2 text-center">
                ...
                >
                  {mfaRequired ? "Two-Factor Verification" : (authMode === "login" ? "Welcome back" : "Create your account")}
                </h2>
                <p
                  style={{
                    fontFamily: "Poppins, sans-serif",
              ...
              {/* User Type Selection - Hide during MFA */}
              {!mfaRequired && (
                <div className="mb-4">
              ...
              >
                {mfaRequired ? (
                  <div className="space-y-3">
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-700 text-center">Authentication Code</label>
                      <input
                        type="text"
                        maxLength={6}
                        value={mfaCode}
                        onChange={(e) => setMfaCode(e.target.value.replace(/\D/g, ''))}
                        className="w-full text-center text-2xl tracking-widest px-4 py-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 outline-none"
                        placeholder="000000"
                        autoFocus
                      />
                    </div>
                ...
                >
                  {isLoading ? "Loading..." : mfaRequired ? "Verify" : authMode === "login" ? "Continue" : "Create Account"}
                </button>
              </form>
            </div>
          </ModalWrapper>
