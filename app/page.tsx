...
export default function HomePage() {
  ...
  const [fetchedAgents, setFetchedAgents] = useState<{ icon: string; color: string; text: string }[]>([]);
  ...
  // Fetch agents for scrolling marquee
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        ...
        // Map agents to visual styles cyclically
        const styles = [
          { icon: "circle", color: "#F05283" },
          { icon: "triangle", color: "#FFC334" },
          { icon: "square", color: "#00AE8E" },
          { icon: "circle", color: "#394fa1" },
          { icon: "triangle", color: "#974095" },
          { icon: "circle", color: "#ed407b" },
          { icon: "square", color: "#FFC334" },
        ];

        const mappedAgents = agentsList.slice(0, 15).map((agent: any, index: number) => ({
        ...
        console.error("Error fetching agents for home page:", err);
      }
    };
  ...
  return (
    <div ref={pageContainerRef} className="flex flex-col" style={{ transform: "translateZ(0)", willChange: "scroll-position", scrollBehavior: "smooth" }}>
      ...
      <section
        className="relative min-h-screen flex items-center justify-center w-full fade-in-section"
        style={{
      ...
      >
        <div className="w-full px-8 md:px-12 lg:px-16 flex justify-center" style={{ position: 'relative', zIndex: 1, pointerEvents: "none" }}>
          <div className="text-center" style={{ pointerEvents: "auto", marginTop: "48px" }}>
            {/* Tangram logo */}
            <div className="mt-0 flex flex-col items-center justify-center scale-in" style={{ marginBottom: "24px", willChange: "transform" }}>
              <Image
            ...
            <div className="mx-auto max-w-5xl overflow-hidden fade-in-section" style={{ marginBottom: "64px", willChange: "opacity, transform" }}>
              {(() => {
                ...
                const agentNames = [
                  { icon: "circle", color: "#F05283", text: "Legal Assistant    " },
                  { icon: "triangle", color: "#FFC334", text: "Sales Copilot" },
                  { icon: "square", color: "#00AE8E", text: "HR Manager" },
                  { icon: "circle", color: "#394fa1", text: "Code Reviewer" },
                  { icon: "triangle", color: "#974095", text: "Marketing Wizard" },
                  { icon: "circle", color: "#ed407b", text: "Data Analyst" },
                  { icon: "square", color: "#FFC334", text: "Security Bot    " },
                  { icon: "triangle", color: "#00AE8E", text: "Support Agent" },
                  { icon: "circle", color: "#394fa1", text: "Finance AI" },
                  { icon: "circle", color: "#394fa1", text: "Finance AI" },
                ];

                // Use fetched agents if available, otherwise fallback to hardcoded
                const displayAgents = fetchedAgents.length > 0 ? fetchedAgents : agentNames;

                const renderTag = (cap: { icon: string, color: string, text: string }, key: string) => (
                  <div
                    key={key}
                ...
                // Ensure enough items for smooth scroll even with few fetched agents
                const duplicatedAgentNames = displayAgents.length < 10
                  ? [...displayAgents, ...displayAgents, ...displayAgents, ...displayAgents]
                  : [...displayAgents, ...displayAgents, ...displayAgents];
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="overflow-hidden relative">
                      ...
                      <div className="flex gap-3 animate-scroll-tags-reverse" style={{ width: "fit-content", animationDuration: "300s" }}>
                        {duplicatedAgentNames.map((cap, idx) => renderTag(cap, `row2-${idx}`))}
                      </div>
                    </div>
                  </div>
                );
      ...
      {/* AI Catalyst Section */}
      <section
        className="fade-in-section py-16 md:py-20 lg:py-24 min-h-[80vh] flex items-center relative"
        style={{
