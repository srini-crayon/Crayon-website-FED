...
async function handleRequest(
  ...
  try {
    ...
    if (method !== 'GET') {
      try {
        ...
        if (arrayBuffer.byteLength > 0) {
          ...
          // Log request body for debugging (only for wishlist endpoints)
          if (path.includes('wishlists') && contentType?.includes('application/json')) {
            try {
              const bodyText = new TextDecoder().decode(arrayBuffer)
              const bodyJson = JSON.parse(bodyText)
              console.log('📤 Wishlist request body:', JSON.stringify(bodyJson, null, 2))
            } catch (e) {
    ...
    try {
      ...
      // Log error responses for wishlist endpoints
      if (path.includes('wishlists') && !response.ok) {
