...
// Default fallback options for target personas
const defaultTargetPersonaOptions = [
...
export default function CustomOnboardPage() {
  ...
  const [targetPersonaOptions, setTargetPersonaOptions] = useState<string[]>(defaultTargetPersonaOptions)
  const [isLoadingTargetPersonas, setIsLoadingTargetPersonas] = useState(false)
    ...
    try {
      ...
      const apiData = {
        ...
        by_persona: (() => {
          ...
          console.log('Onboarding page - by_persona value being sent:', byPersonaValue, 'from personas:', formData.targetPersonas)
          return byPersonaValue
        })(),
        features: formData.keyFeatures.length > 0 ? formData.keyFeatures.join("; ") : "", // Join with semicolon to match DB format
  ...
  // Fetch target personas from API
  useEffect(() => {
    const fetchTargetPersonas = async () => {
      setIsLoadingTargetPersonas(true)
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch target personas: ${response.status}`)
        }
        
        const data = await response.json()
        
        // Extract target_personas from response
        const targetPersonas = Array.isArray(data.target_personas) 
          ? data.target_personas 
          : (data.target_personas ? [String(data.target_personas)] : defaultTargetPersonaOptions)
        
        // Ensure we have unique values only
        const uniquePersonas = [...new Set(targetPersonas.filter(Boolean))]
        
        if (uniquePersonas.length > 0) {
          setTargetPersonaOptions(uniquePersonas)
        }
      } catch (error) {
        console.error('Error fetching target personas:', error)
        // Keep default values on error
        setTargetPersonaOptions(defaultTargetPersonaOptions)
      } finally {
        setIsLoadingTargetPersonas(false)
      }
    }
